...
Install plugins by copying a folder into `plugins/`, then open **Plugins** in the web UI to refresh, enable/disable, edit config, or restart a plugin. The bundled `plugins/echo/` plugin is a minimal example using `{"prefix": "/echo", "respond_to_self": false}`.
